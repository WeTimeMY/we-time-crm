...
# ================== RECORD SPENDING (Fixed Clearing) ==================
elif menu == "💰 Record Spending":
    st.subheader("💰 Record Spending")
    
    # Use session state to control form reset
    ...
    if st.session_state.reset_form:
        ...
        phone_input = st.text_input("Customer Phone Number", key="phone")
        if phone_input:
            ...
            if not matching.empty:
                ...
                amount = st.number_input("Spending Amount (RM)", min_value=0.0, step=1.0, key="amount")
                outlet = st.selectbox("Outlet", outlets, key="outlet")
                ...
                if st.button("Record Spending & Add Points", type="primary"):
                    if amount > 0:
                        ...
                        st.success(f"🎉 RM{amount} recorded for **{customer['name']}** → +{points} points!")
...
# ================== ADJUST POINTS ==================
    ...
    if phone_input:
        ...
        if not matching.empty:
            ...
            adjustment = st.number_input("Points to Adjust", value=0, step=1, help="Negative to deduct")
...
# ================== BIRTHDAY NOTIFICATIONS ==================
    ...
    if customers.empty:
        ...
        st.info("Birthday notification is active.")

st.sidebar.info("**We Time CRM v3.5** - Full Version")
